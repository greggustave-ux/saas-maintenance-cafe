...
-- 3. Supprime temporairement le trigger s'il existe déjà pour éviter de bloquer le backfill.
...
-- 6. Crée la fonction trigger `handle_service_call_reference_number` pour auto-générer les numéros de référence formatés (SC-YYYY-000001) et empêcher toute modification.
-- 7. Crée le trigger `trg_service_call_reference_number` sur la table `service_calls`.
--
...
-- 3. Supprimer temporairement le trigger s'il existe déjà pour que l'UPDATE du backfill ne soit pas refusé
...
-- 5. Rendre la colonne obligatoire (NOT NULL) et UNIQUE
