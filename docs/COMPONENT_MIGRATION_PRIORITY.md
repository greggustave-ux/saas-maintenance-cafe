...
- **ServiceCallCard (v1.0.0):** Migrated as first operational card candidate. Fully refactored card and skeleton layout to consume design-system HSL color scales, spacing variables, standard typography token values, custom borders/shadows, and touch-target minimum dimensions (>= 44px). All existing technician navigation, delete actions, state selections, and database hooks are preserved, ensuring complete functional regression protection.
- **DispatchKanbanCard (v1.0.0):** Migrated as second operational card candidate. Swapped hardcoded style properties in `DispatchCard` for HSL theme tokens, standardizing border/shadow values, margins, and layout widths. Integrated static `StatusBadge` and `PriorityBadge` elements into the card header while maintaining the interactivity of status, priority, and technician selectors. Restructured select target dimensions to >= 44px to satisfy mobile responsiveness constraints without altering database schemas or board sorting/filtering workflows.
- **ServiceCallCreateEditForm (v1.0.0):** Migrated as first operational form candidate. Replaced custom styling blocks and inputs in `NewServiceCallForm` with standard design-system form components (`DSInput`, `DSTextarea`, `DSSelect`, `DSLabel`, `DSFormSection`). Enforced touch-target sizes of >= 44px on all input types, aligned grid gutters to design-system spacing parameters, and verified functional continuity of submission triggers and error validations. No Supabase schemas or query codes were altered.
