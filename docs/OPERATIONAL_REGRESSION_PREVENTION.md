...
- **DispatchKanbanCard (v1.0.0):** Migrated as the second operational card. Refactored the core `DispatchCard` to use HSL colors, design-system spacing variables, typography scales, custom radii, and shadow variables. Re-engineered all interactive dropdown selectors to satisfy mobile touch-target dimensions (>= 44px). Added robust visual fallbacks for empty technician assignments, serial numbers, client names, and service description fields. Verified layout rendering, theme adaptability, and dispatcher workflow continuity without modifying Supabase queries or schemas.
